...
{  
    ...
    if(uiReceiveLen <= 0)
    {
        XiaoHG_Log(LOG_FILE, LOG_LEVEL_ERR, errno, "Client disconnected, socket id: %d", pConn->iSockFd);
        return;
    }
...
ssize_t CSocket::RecvProc(LPCONNECTION_T pConn, char *pSendBuff, ssize_t uiBuffLen)   
{
    ...
    if(n < 0) 
    {
        ...
        if(errno == EAGAIN || errno == EWOULDBLOCK)
        {
            ...
            XiaoHG_Log(LOG_FILE, LOG_LEVEL_NOTICE, errno, "errno == EAGAIN || errno == EWOULDBLOCK failed");
            return XiaoHG_ERROR;
        }
        ...
        {
            /* Not treated as an error, simply return */
            XiaoHG_Log(LOG_FILE, LOG_LEVEL_NOTICE, errno, "errno == EINTR failed");
            return XiaoHG_ERROR;
        }
        ...
        if(errno == ECONNRESET)  
        {
            ...
            XiaoHG_Log(LOG_FILE, LOG_LEVEL_ERR, errno, "receive message failed");
        }
        else
        {
            /* error */
...
ssize_t CSocket::SendProc(LPCONNECTION_T pConn, char *pSendBuff, ssize_t uiSendLen)  
{
    ...
    for ( ;; )
    {
        ...
        if(n == 0)
        {
            ...
            XiaoHG_Log(LOG_FILE, LOG_LEVEL_ERR, errno, "send failed");
            return 0;
        }
...
void CSocket::EpollEventWriteRequestHandler(LPCONNECTION_T pConn)
{
    ...
    {
        /* Send buffer full */
        XiaoHG_Log(LOG_FILE, LOG_LEVEL_ERR, errno, "Send buffer full");
        return;
    }
